...
public class AdminUploadWindow extends SGDUSecureWindow {
	...
	public void helpDoc(){
		...
		try {
			Messagebox.show("Ao enviar um documento de texto, " +
					...
					"Enviar\". Mas atenção, os formatos de arquivo aceitos pelo " +
					"sistema são:\n" +
					FileSupportVerificator.DocDescription(),
	...
	public void helpImage(){
		...
		try {
			Messagebox.show("Ao enviar um documento digitalizado, o sistema " +
					"extrai o texto automaticamente e o armazena para que possa " +
					"ser consultado posteriormente. Primeiro selecione o número " +
					"de imagens que compõem o documento. Depois clique em \"Upload\" " +
					"e selecione cada uma das imagens. " +
					"Finalmente clique em \"Enviar\". Mas atenção, certifique-se de " +
					"que as páginas escaneadas do documento estejam legíveis e em " +
					"boa qualidade. Os formatos de imagem aceitos pelo sistema são:\n" +
					FileSupportVerificator.ImageDescription(),
					"Ajuda",Messagebox.OK, Messagebox.INFORMATION);
		} catch (InterruptedException e) {
			e.printStackTrace();
		}
			...
			for(int i=0; i<imagesaux.length; i++){
				if(imagesaux[i] != null){
					if( !FileSupportVerificator.isImageValid( imagesaux[i].getName()) ){
						Messagebox.show(imagesaux[i].getName() + " Esse tipo de arquivo não " +
								"é suportado. Clique no botão \"?\" para mais informações.",
								"Arquivo inválido", Messagebox.OK, Messagebox.ERROR);
						return;
					}
				}
	...
	public void send() throws Exception{
		...
		boolean isDocMediaType =
			((Radiogroup)getFellow("typeFileRadioGroup")).getSelectedIndex() != 1;
		...
		if( fail ){
			try{
				Messagebox.show("Você não fez upload de nenhum" +
						(isDocMediaType? " documento." : "a imagem."),
		...
		if( isDocMediaType ){
			...
			if( FileSupportVerificator.isPDFType(doc.getName()) ){
				...
				dbDocManager.insertDocument( title, description, date,
						...
						dbAccGate.getAdministrator().getLogin(), content,
						Document.PORTABLE_DOCUMENT_FILE_TYPE);
			...
			// HTML
				...
				dbDocManager.insertDocument( title, description, date,
						new Date(System.currentTimeMillis()),
						dbAccGate.getAdministrator().getLogin(), content,
						Document.HIPERTEXT_MARKUP_TYPE);
		...
		// INSERE IMAGEM
			...
			dbDocManager.insertDocument( title, description, date,
					new Date(System.currentTimeMillis()),
					dbAccGate.getAdministrator().getLogin(), content, Document.IMAGE_TYPE );
			...
			case FileSupportVerificator.DOCX_TYPE:
				try{
					...
					try{
						...
						dbDocManager.insertFile(doc.getName().replace(' ', '_'),
								Reader2StringConverter.get(doc.getReaderData()).getBytes() );
					}
				}
			break;
